<p align="center">
  <img src="docs/exe_logo.png" alt="exe logo" width="320">
</p>

**exe** researches the live web with Tavily, remembers your voice and glossary via Mem0, and publishes a cited illustrated explainer to GitHub. The starter agent script prints an answer and forgets; this ships a durable artifact you can share, trace, and evaluate.
...
cd exe
uv sync
cp .env.example .env   # fill in all keys below (required for the full workflow)
uv run exe "What are sea lions?"
```

All keys below are required for the workflow this repo was built and verified against: research, memory, illustrated article, GitHub publish, and LangSmith tracing. Use `--skip-publish` only if you intentionally want a local artifact without pushing to GitHub.
...
| `MEM0_API_KEY`       | Yes      | Hosted Mem0 for style + glossary                           | [app.mem0.ai](https://app.mem0.ai)                                                                                                          |
| `GEMINI_API_KEY`     | Yes      | Ian Xiaohei illustrations                                  | [aistudio.google.com/apikey](https://aistudio.google.com/apikey)                                                                            |
| `GEMINI_IMAGE_MODEL` | No       | Image model (default `gemini-3.1-flash-lite-image`)        | Same as Gemini key                                                                                                                          |
| `GITHUB_TOKEN`       | Yes      | Publish via PyGithub                                       | [github.com/settings/tokens](https://github.com/settings/tokens) — fine-grained token needs **Contents: Read and write** on the target repo |
| `GITHUB_REPO`        | No       | `owner/repo` (defaults to git remote or `rockangator/exe`) | N/A                                                                                                                                         |
| `LANGSMITH_TRACING`  | Yes      | Enable tracing (`true`)                                    | [smith.langchain.com](https://smith.langchain.com)                                                                                          |
| `LANGSMITH_API_KEY`  | Yes      | LangSmith API key                                          | [smith.langchain.com/settings](https://smith.langchain.com/settings)                                                                        |
...
CLI (agent/cli.py)
  ...
  ├─ generate ─────► Nebius agent (+ illustrate tool)
...
├── docs/
│   ├── demo/               # README screenshots + LangSmith trace
│   ├── exe_logo.png        # project logo
